...
# =========================================================
# ✅ 결측 치환 정책 (p_call/p_rl 산출용)
#   - Age/Gender: 기존 유지
#   - missing_data 코드 있는 범주형: NaN -> missing_data 코드
#   - missing_data 코드 없는 범주형(Protective): NaN -> unknown 코드
#   - Vital: NaN -> unchecked(0)
# =========================================================
...
# 범주형: NaN -> missing_data 코드 (코드북 기준, preprocess 내부 map과 일치해야 함)
# (각 코드 값은 아래에서 실제 one-hot map에 이미 포함된 missing_data 코드 값으로 맞춤)
_MISSING_DATA_FILL = {
    # Injury time
...
# missing_data 코드가 없는 범주형: NaN -> unknown 코드
_UNKNOWN_FILL = {
...
def preprocess_from_raw(df: pd.DataFrame) -> pd.DataFrame:
    """
    기존 피처 생성 로직은 그대로 유지하고,
    ✅ raw 결측을 코드북 정책대로 치환하는 단계만 선행 적용한다.
    """
    # ✅ 결측치 정책 적용 (여기만 추가됨)
    df = _apply_missing_policy(df)

    # ✅ OCS등록번호 유지 (매칭 키)
    record = df[["OCS등록번호"]].copy()
    record["내원일시"] = df["내원일시"]
    ...
    # Age normalize (Age_code 1..??)  ✅ 기존 유지 (NaN이면 0으로 들어감)
    ...
    # Gender -> male  ✅ 기존 유지 (NaN이면 male=0)
    ...
    # Protective  ✅ missing_data 없음 → 결측은 unknown(2)로 이미 치환됨
...
def get_feature_matrix(processed: pd.DataFrame):
    ...
    X = processed.drop(columns=["OCS등록번호", "내원일시", "Mortality"])
    return X, y
