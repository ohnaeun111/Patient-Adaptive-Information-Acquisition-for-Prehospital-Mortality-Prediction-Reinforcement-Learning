...
def validate_required_files(paths: Paths) -> None:
    required = [paths.ALL_XLSX, paths.CALL_XLSX, paths.MODEL_PKL]
    missing = [path for path in required if not path.is_file()]
    if missing:
        formatted = "\n".join(f"  - {path}" for path in missing)
        raise FileNotFoundError(
            "Required private training files were not found:\n"
            f"{formatted}\n"
            "Place the approved files in the repository's data directory."
        )
...
def main():
    ...
    df_all_aligned, df_call_aligned, forced_age, forced_gender, match_rate = align_call_to_all_by_ocs_date(
        df_all_raw, df_call_raw, ocs_col="key", visit_col="date"
    )
